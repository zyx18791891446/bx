...
//			systemRealm.clearAllCachedAuthorizationInfo();
		...
		if(!officeId.equals("1")){
			...
			if(expert==null){//表示插入
				expert = new Expert();
				expert.setAge("25");
				expert.setExpertname(user.getName());
				expert.setSex("男");
				String specialty = officeService.get(user.getOffice().getId()).getName();
				expert.setSpecialty(specialty);
				expertService.save1(expert,id);
			}else{//表示修改
				expert.setExpertname(user.getName());
				String specialty = officeService.get(user.getOffice().getId()).getName();
				expert.setSpecialty(specialty);
	...
	@Transactional(readOnly = false)
	public void updateUserInfeo(User user) {
		user.preUpdate();
