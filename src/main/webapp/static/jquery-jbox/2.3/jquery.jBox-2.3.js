...
(function (a) {
	a.jBox = function (b, c) {
		...
		var d =  !a.support.leadingWhitespace;
		var e = a('#' + c.id);
		if (e.length > 0x0) {
		...
		if (!c.isTip && !c.isMessager && !c.showScrolling) {
			a(a.support.leadingWhitespace ? 'body' : 'body').attr('style', 'overflow:hidden;padding-right:17px;')
		};
		...
		j.push('<div class="jbox-help-button jbox-button-panel" style="height:50px;padding:5px 0 5px 0;display:none;"></div>');
		...
		if (g) {
			...
			j.push('<div class="jbox-title' + (c.showIcon == true ? ' jbox-title-icon' : (c.showIcon == false ? '' : ' ' + c.showIcon)) + '" style="float:left; width:' + i + '; line-height:' + (a.support.leadingWhitespace ? 0x19 : 0x18) + 'px; padding-left:' + (c.showIcon ? 0x12 : 0x5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (c.title == '' ? '&nbsp;' : c.title) + '</div>');
			j.push('</div>')
		};
		...
		var u = a.support.leadingWhitespace ? '' : '';
		a.each(b, function (N, O) {
			...
			r.push('<div class="jbox-button-panel" style="height:50px;padding:10px;text-align: right;' + (P ? '' : 'display:none;') + '">');
			if (!c.isTip) {
				r.push('<span class="jbox-bottom-text" style="float:left;display:block;line-height:25px;"></span>')
			};
			a.each(O.buttons, function (T, U) {
				r.push('<button class="jbox-button btn btn-sm" value="' + U + '" style="' + u + '">' + T + '</button>')
			});
			r.push('</div></div>')
		});
		...
		a.each(b, function (N, O) {
			...
			P.find('.jbox-button-panel button:eq(' + O.buttonsFocus + ')').addClass(' btn btn-primary btn-sm jbox-button-focus')
		});
		var v = function () {
			n.css({
				top : l.scrollTop()
		...
		var I = function () {
			if (!c.isTip && !c.isMessager) {
				if (a('.jbox-body').length == 0x1) {
					a(a.support.leadingWhitespace ? 'body' : 'body').removeAttr('style')
				};
				J();
				// 解决框架内object和applet被遮住问题
				fade(a('body, iframe'), function(obj, ifr){
	...
	a.jBox.prompt = function (b, c, d, e) {
		...
		var g = e.icon == 'none' ? 'height:auto;' : 'min-height:30px;' + ((a.support.leadingWhitespace && parseInt(a.support.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;' : 'height:auto;');
		var h = [];
		h.push('html:');
	...
	a.jBox.messager = function (b, c, d, e) {
		...
		var h = e.icon == 'none' ? 'height:auto;' : 'min-height:30px;' + ((a.support.leadingWhitespace && parseInt(a.browser.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;' : 'height:auto;');
		var i = [];
		i.push('html:');
